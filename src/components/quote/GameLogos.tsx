...
import type { QuoteGame } from "@/data/quote";
...
function LogoSvg({ className, children }: { className?: string; children: ReactNode }) {
  return (
    <svg
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden
      className={clsx("h-full w-full fill-current", className)}
    >
      {children}
    </svg>
  );
...
export function GameLogo({ game, className }: GameLogoProps) {
  switch (game) {
    case "minecraft":
    ...
    default:
      return <Gamepad2 className={clsx("h-full w-full", className)} strokeWidth={1.5} aria-hidden />;
  }
}
