...
import {
  buildPurchaseCustomerEmail,
  buildPurchaseEmailContent,
  isValidPurchaseSubmission,
  PURCHASE_TO_EMAIL,
} from "@/lib/purchaseEmail";

type PurchaseSendBody = {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const data = (req.body as PurchaseSendBody).data;
  if (!data) {
  ...
  const ownerEmail = buildPurchaseEmailContent(data, price!);
  ...
  try {
    ...
    if (ownerResult.error) {
      return res.status(502).json({ error: ownerResult.error.message ?? "failed to send purchase email" });
    }

    if (data.contactMethod === "email") {
      const customerEmail = buildPurchaseCustomerEmail(data, price!);
